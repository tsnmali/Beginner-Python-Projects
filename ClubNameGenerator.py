# A simple program to generate a unique club name.
print("Welcome to Club Name Generator!")

# Ask the user for the city they grw up in and storing the response in a variable.
user_city = input("What's the name of the city you grew up in?\n")

# As the user for their pet and storing the response in a variable.
user_pet = input("What's the name of your pet?\n")

# Using string concatenation to combine the user's city and pet name to create a unique club name!
print("Your band name could be " + user_city + " " + user_pet + ". How rad!")
